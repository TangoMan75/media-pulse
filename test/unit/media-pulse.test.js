...
import { formatNumber, getValue, parseHTML, calculateAverages, calculateTotals, loadStatsFromJson, saveStatsToJson, isDataFresh } from '../../media-pulse.js';
...
describe('formatNumber', () => {
  ...
  it('should round the input number', () => {
    assert.strictEqual(formatNumber(5000.7), '5001');
    assert.strictEqual(formatNumber(5000.4), '5000');
    assert.strictEqual(formatNumber(123456.7), '123 457');
    assert.strictEqual(formatNumber(123456.4), '123 456');
...
describe('generate SVGs', () => {
  it('should generate SVG even if data length < 12', async () => {
    const statsData = [{
      id: 'lemonde',
      url: 'https://lemonde.fr',
      lastUpdated: new Date().toISOString(),
      metrics: [
        { period: '2026-03', visits: 214316867, pages: 556432131 }
      ]
    }];
    const logoContent = '<svg width="200" height="50" viewBox="0 0 200 50"></svg>';

    mock.method(fs, 'readFileSync', () => JSON.stringify(statsData));
    mock.method(fs, 'writeFileSync', () => {});
    mock.method(fs, 'mkdirSync', () => {});
    mock.method(fs, 'existsSync', () => true);
    mock.method(fs.promises, 'readFile', async () => logoContent);

    try {
      const { runBuildCommand } = await import('../../media-pulse.js');
      await runBuildCommand();
      const writeCalls = fs.writeFileSync.mock.calls;
      assert.ok(writeCalls.some(c => c.arguments[0].endsWith('.svg')), 'SVG should be generated even with incomplete data');
    } finally {
      mock.restoreAll();
    }
  });

  it('should fall back to _logo.svg when _logo_invert.svg is missing', async () => {
    const statsData = [{
      id: 'lemonde',
      url: 'https://lemonde.fr',
      lastUpdated: new Date().toISOString(),
      metrics: [
        { period: '2026-03', visits: 500000, pages: 2000000 }
      ]
    }];
    const logoContent = '<svg width="200" height="50" viewBox="0 0 200 50"></svg>';
    ...
    mock.method(fs.promises, 'readFile', async () => {
      callCount++;
      if (callCount === 1) {
        throw new Error('ENOENT: _logo_invert.svg not found');
      }
      return logoContent;
    });
    ...
    try {
      ...
      await runBuildCommand();
      const writeCalls = fs.writeFileSync.mock.calls;
      assert.ok(writeCalls.some(c => c.arguments[0].endsWith('.svg')), 'SVG should be generated with fallback logo');
      assert.strictEqual(callCount, 3, 'Should attempt dark (fail→fallback), then light logo');
    } finally {
      mock.restoreAll();
    }
  });
